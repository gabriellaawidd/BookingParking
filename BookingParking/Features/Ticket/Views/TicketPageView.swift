...
struct TicketPageView: View {
        ...
        ZStack(alignment: .bottom) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        if selectedSegment == .schedule {
                            if bookingStore.scheduleBookings.isEmpty {
                                emptyState(text: "No upcoming reservation")
                            } else {
                                ForEach(bookingStore.scheduleBookings) { booking in
                                    TicketScheduleCard(booking: booking)
                                }
                            }
                        } else {
                            if bookingStore.historyBookings.isEmpty {
                                emptyState(text: "No history yet")
                            } else {
                                ForEach(bookingStore.historyBookings) { booking in
    ...
    private var segmentedControl: some View {
        HStack(spacing: 0) {
            SegmentButton(title: "Schedule", isSelected: selectedSegment == .schedule) {
                withAnimation(.easeInOut(duration: 0.2)) { selectedSegment = .schedule }
            }
            SegmentButton(title: "History", isSelected: selectedSegment == .history) {
                withAnimation(.easeInOut(duration: 0.2)) { selectedSegment = .history }
            }
        }
        .background(Color(.systemGray6))
        .clipShape(Capsule())
